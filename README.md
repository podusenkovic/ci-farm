...
| `ci monitor` | Живой дашборд мониторинга slave-устройств |
...
Также поддерживается сокращённый режим — любая команда, не являющаяся подкомандой, выполняется на slave (с предварительной синхронизацией проекта):
...
| Опция | Описание |
|-------|----------|
| `--interval`, `-i` | Интервал обновления в секундах (default: 5) |

`ci monitor` отображает в реальном времени для каждого slave:

- Загрузка CPU (на основе load average)
...
Панели цветокодированы: зелёный — свободен, жёлтый — занят сборкой, красный — оффлайн. Выход по `Ctrl+C`.
